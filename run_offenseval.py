...
""" Finetuning the library models for sequence classification on OffensEval"""
    ...
    Examples::
        ...
        if labels is not None:
            if self.num_labels == 3:
                weights = [1, 0.45951336258, 0.171519744] # for final training document

            else:
                weights = [0.2180936323532808, 1] # for final training document
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    # the weird 'custom threshold' thing we did to balance precision/recall on dev data....
    if args.task == 'task_b':
        for i, (zero, one) in enumerate(preds):
            # if one > -1:
            if abs(zero-one) < 3.75 or abs(one-zero) < 3.75:
                preds[i][0] = 0
                preds[i][1] = 1
    elif args.task == 'task_a':
        for i, (zero, one) in enumerate(preds):
            if (abs(zero-one) < 2.75 or abs(one-zero) < 2.75) and args.language == 'danish':
                preds[i][0] = 0
                preds[i][1] = 1
            if (abs(zero-one) < 2.25 or abs(one-zero) < 2.25) and args.language == 'arabic':
                preds[i][0] = 0
                preds[i][1] = 1
            if (abs(zero-one) < 1.95 or abs(one-zero) < 1.95) and args.language == 'turkish':
                preds[i][0] = 0
                preds[i][1] = 1
            if (abs(zero-one) < 0.5 or abs(one-zero) < 0.5) and args.language == 'greek':
                preds[i][0] = 0
                preds[i][1] = 1
    ...
    output_eval_file = os.path.join(eval_output_dir,  args.language + "_eval_results.txt")
    ...
    with open(args.output_dir + '/' + args.language + '_dev_preds.txt', 'w') as outfile:
...
def predict(args, model, tokenizer, prefix=""):
    ...
    results = {}
    eval_dataset = load_and_cache_examples(args, tokenizer, test=True)
    ...
    if args.task == 'task_b':
        for i, (zero, one) in enumerate(preds):
            ...
            if abs(zero-one) < 3.75 or abs(one-zero) < 3.75:
                preds[i][0] = 0
                preds[i][1] = 1
    elif args.task == 'task_a' and args.language != 'english':
        for i, (zero, one) in enumerate(preds):
            # if one > -1:
            if (abs(zero-one) < 2.75 or abs(one-zero) < 2.75) and args.language == 'danish':
                preds[i][0] = 0
    ...
    id2preds = {val: preds[i] for i, val in enumerate(ids)}
    preds = [id2preds[val] if val in id2preds else [] for i, val in enumerate(ids)]
    with open(args.output_dir + '/' + args.language + '_test_preds_with_ids_' + args.task + '.tsv', 'w') as outfile:
        for idx, doc_id in enumerate(ids):
            if args.language == 'english' and args.task == 'task_a':
                line = 'A' + str(doc_id) + "\t"
            elif args.language == 'english' and args.task == 'task_b':
                line = 'BC' + str(doc_id) + "\t"
            else:
                line = str(doc_id) + "\t"
            if preds[idx] == 0:
                ...
                if args.task == 'task_b':
                    ...
                    line += 'OFF' + "\n"
            outfile.write(line)
    with open(args.output_dir + '/' + args.language + '_test_preds_' + args.task + '.tsv', 'w') as outfile:
        for idx, doc_id in enumerate(ids):
            line = ''
            if preds[idx] == 0:
                line += 'NOT' + "\n"
...
def load_and_cache_examples(args, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = processor.get_dev_examples(args.data_dir) if evaluate \
            else processor.get_test_examples(args.data_dir) if test else processor.get_train_examples(args.data_dir)
    ...
    all_doc_ids = torch.tensor([int(f.guid) for f in features], dtype=torch.long)


    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_doc_ids)
    return dataset
